...
amd.init = function(conf) {
  ...
  function lookup(info, file) {
    info = info.rest ? info : fis.util.query(info);
    ...
    if ((!test || !test.file) && (id === '.' || id.substring(id.length - 1) === '/') && conf.packagers) {
      ...
      if (pkg) {
        test = findResource(pkg.main || 'main', folder);
      }
    }
    ...
    if ((!test || !test.file) && /^([^\/]+)(?:\/(.*))?$/.test(id)) {
      ...
      if (pkg) {
        if (pkg.isFISID) {
          ...
          test = findResource(subpath || pkg.main || 'main', pkg.folder);
        }
      } else if ((dirs = paths[prefix])) {
        for (var i = 0, len = dirs.length;
          (!test || !test.file) && i < len; i++) {
          test = subpath ? findResource(subpath, path.join(baseUrl, dirs[i])) : findResource(dirs[i], baseUrl);
        }
      }
    }
...
amd.test = function(info) {
  ...
  if ('undefined' !== typeof file.module && file.module !== 'amd' || !rDefine.test(content)) {
    return false;
  }
  ...
  try {
    ...
    isAMD = ret.modules && ret.modules.length;
  } catch (e) {
...
function autowrap(info, conf) {
  ...
  if (shim) {
    file.isMod = true;
  } else if ('undefined' === typeof file.isMod) {
    // 自动判断目标文件 isMod 是否为模块化代码。
    file.isMod = /(?:\bmodule\.exports\b)|(?:\bexports\.)|(?:\bexports\[])/i.test(content);
  }
...
function parse(file, content, conf) {
  ...
  if (modulesCount > 1) {
    fis.log.warning('You can define only one module in the file `%s`.', file.subpath);
    return content;
  }

  file.isMod = !!modulesCount;
  ...
  travelRet(ret, function(node) {
    ...
    if (module) {
      if (module.id) {
        fis.log.warning('Please don\'t specify the id `%s` of the module in `%s`. And we will erase it anyway.', module.id, file.subpath);
      }
      ...
      if (module.deps && module.deps.length) {
        ...
        module.deps.forEach(function(elem) {
          ...
          deps.push(info.quote + lang.jsRequire.ld + moduleId + lang.jsRequire.rd + plugins + info.quote);
      ...
      if (node.requires && node.requires.length) {
        node.requires.forEach(function(item) {
          ...
          inserts.push({
            start: start,
            len: elem.raw.length,
            content: info.quote + lang.jsRequire.ld + moduleId + lang.jsRequire.rd + plugins + info.quote
          });
